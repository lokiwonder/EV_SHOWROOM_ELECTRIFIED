import { useElectrifiedMenuStore, useElectrifiedStore, useLanguageStore, useSettingStore } from 'src/stores';
import { VehicleMenuView } from 'src/screens';

import './style.css';
import { LanguageIcon, LanguageActiveIcon, Logo, MenuIcon } from 'src/assets/images'
    ...
    <>
      ...
      { language_menu && (
        ...
        <div className="language-list">
          {setting && setting.languages.map((item: any) => (
            <p className={language === item ? 'language-item-active' : 'language-item'} onClick={() => onLanguageSelectHandler(item)}>{LANGUAGE(item)}</p>
          ))}
        </div>
      </div>
      ) }
